    ...
    for k in range(tgt_centroids.shape[0]):
        ...
        for j in range(len(src_centroids)):
            ...
            distances.append(distance.euclidean(
                tgt_centroids[k], src_centroid_loc))  # calculate distances
        matching_clusters.append((k, np.argmin(
            distances), distances[np.argmin(distances)]))  # find closest centroid

    return matching_clusters


def mammogram_clustering(images, params, artifact_mask=True, cluster_to_remove=-1, second_round_cluster=-1, mbkm_centroids=None, plot=True):
    artifact_masks = [init_artifact_mask(
        i, params['color_bounds']) for i in images]
    valid_img_pixels = np.concatenate(
        [images[i][artifact_masks[i] == False].reshape(-1, 1) for i in range(len(images))])

    mbkm = MiniBatchKMeans(n_clusters=params['k'],
                           random_state=2316,
                           init=params['init_centroids'],
                           batch_size=params['batch_size'])
    mbkm.fit(valid_img_pixels)
    labelled_images = [mbkm.predict(
        i.reshape(-1, 1)).reshape(i.shape[0], i.shape[1]) for i in images]
    ...
    if cluster_to_remove != -1:
        cluster_pairs = find_closest_centroids(
            mbkm.cluster_centers_, mbkm_centroids)
        ...
        if second_round_cluster != -1:
            ...
            if second_round_cluster != cluster_to_remove:
                if np.where(labelled_images[0] == second_round_cluster)[1][0] > np.where(labelled_images[0] == cluster_to_remove)[1][0]:
                    tmp = cluster_to_remove
                    cluster_to_remove = second_round_cluster
                    second_round_cluster = tmp

                pixels = [images[i][labelled_images[i] ==
                                    second_round_cluster].reshape(-1, 1) for i in range(len(images))]

                ll_mbkm = MiniBatchKMeans(n_clusters=2,
                                          random_state=2316,
                                          init=params['init_centroids'],
                                          batch_size=params['batch_size'])
                ...
                for i in range(len(labelled_images)):
                    labelled_images[i][labelled_images[i] == second_round_cluster] = [
                        cluster_to_remove if j == cluster_pairs[cluster_to_remove][1] else params['k'] for j in ll_labels[i]]
...
if __name__ == "__main__":
    ...
    images = [image_preprocess(cv2.imread(
        readinpath + i, cv2.IMREAD_GRAYSCALE)) for i in onlyfiles]
    ...
    images, labelled_images, mbkm, ll_mbkm = mammogram_clustering(
        images, params=params, cluster_to_remove=2, second_round_cluster=0, mbkm_centroids=np.load('./notebooks/mbkm_clusters.npy'), plot=True)
