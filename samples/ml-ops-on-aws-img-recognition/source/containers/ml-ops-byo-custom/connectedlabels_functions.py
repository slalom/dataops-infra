...
def generate_connectedlabels_variables(
...
def plot_connectedlabels_partition(
    image,
    thresh,
    filteredchoice,
    binary,
...
def plot_pectoralmuscle_segmentation(
    chosen_method,
    image,
    thresh,
    filteredchoice,
    ...
    ax[2].scatter(
        indexinflection_topdown_x, indexinflection_topdown_y, s=80, c="red", marker="o"
    )
    ax[2].axis("off")
...
def remove_pectoralmuscle(image, otsu_cropwidth, otsu_cropheight, blob_sigmasetting):
    ...
    (
        ...
        filteredchoice,
        subimage,
    ) = generate_connectedlabels_variables(
        image, otsu_cropwidth, otsu_cropheight, blob_sigmasetting
    )
    ...
    (
        ...
        indexinflection_topdown_y,
        maxtoprowpoint,
    ) = check_breasttissue_linemethod(blobs_label1, image)

    if (percent_outside > 0.1) & (np.array(linecroppedimage).sum() != 0):
...
def main():
    ...
    (
        ...
        indexinflection_topdown_y,
        maxtoprowpoint,
    ) = check_breasttissue_linemethod(binarymask, image)
    ...
    np.save(image_muscleremoved_npyimagepath + savefilename, image_nopectoralmuscle)
    # change for saving in s3 bucket

    plot_pectoralmuscle_segmentation(
        chosen_method,
        image,
        thresh,
        filteredchoice,
